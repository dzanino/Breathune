/* breathune.eu — site.js
   ...
   Hidden panel: click "©" (or "Breathune" on Support/Privacy) in the footer three times quickly. */
(function () {
  ...
  if (!isBot && store('l', 'br-own') !== '1' && !store('s', 'br-v')) {
    store('s', 'br-v', '1');
  ...
  function open() {
    ...
    if (store('l', 'br-own') === null) store('l', 'br-own', '1'); // first open: stop counting this device
    panel = document.createElement('div');
    panel.setAttribute('role', 'dialog');
    panel.style.cssText = 'position:fixed;right:16px;bottom:16px;z-index:9999;max-width:320px;width:calc(100% - 32px);' +
      'background:#0d3a4a;color:#eaf4f6;border-radius:14px;padding:16px 18px;font:14px/1.5 -apple-system,Helvetica,Arial,sans-serif;' +
      'box-shadow:0 18px 44px rgba(0,0,0,.35)';
    panel.innerHTML =
      '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">' +
        '<b style="font-size:15px">Návštevnosť breathune.eu</b>' +
        '<button type="button" aria-label="Zavrieť" style="background:none;border:0;color:#9fc4e6;font-size:20px;cursor:pointer;line-height:1">×</button>' +
      '</div>' +
      '<div data-x="body" style="color:#c9d7e5">Načítavam…</div>' +
      '<label style="display:flex;gap:8px;align-items:flex-start;margin-top:10px;font-size:12.5px;color:#9fb4c8;cursor:pointer">' +
    ...
    cb.checked = store('l', 'br-own') === '1';
    cb.onchange = function () { store('l', 'br-own', cb.checked ? '1' : '0'); };
    ...
    Promise.all(keys.map(get)).then(function (v) {
      ...
      panel.querySelector('[data-x="body"]').innerHTML =
        ...
        '</div>' +
        '<div style="margin-top:8px;font-size:12px;color:#8aa3ba">Počíta sa od ' + since + ' (' + days + (days === 1 ? ' deň' : days < 5 ? ' dni' : ' dní') +
